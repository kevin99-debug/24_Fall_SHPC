...
void matmul(const float *A, const float *B, float *C, int M, int N, int K) {
  ...
  //setup OpenCL global/local work size
  size_t gws[2] = { (size_t)(M + BLOCK_SIZE * NUM_ELEM_PER_THREAD - 1) / (BLOCK_SIZE * NUM_ELEM_PER_THREAD) * BLOCK_SIZE,
                    (size_t)(N + BLOCK_SIZE * NUM_ELEM_PER_THREAD - 1) / (BLOCK_SIZE * NUM_ELEM_PER_THREAD) * BLOCK_SIZE };
  size_t lws[2] = {BLOCK_SIZE / NUM_ELEM_PER_THREAD, BLOCK_SIZE / NUM_ELEM_PER_THREAD};
  ...
  //After running kernel, send it back to CPU.
...
void matmul_finalize() {
  //Need to free all resources allocated
